...
API 启动时会幂等导入 25 道数据结构代码题，共 302 个测试点：

- 2009-2025 年 408 原卷中明确要求使用 C/C++/Java 实现算法或函数的 17 道题，恰好每年一道，共 188 个测试点；
- 2026 王道计算机 408 八套卷中明确要求实现算法的 8 道题，每卷一道，共 114 个测试点。

手工计算、证明、算法执行模拟和只要求分析固定数据的综合应用题不会改编进 OJ。王道八套卷中与代码题并列的数据结构应用题也不导入。每道受管题均包含：
...
王道模拟题按 2026 八套卷逐卷核验。8 道入库题均为第 42 题，考点依次为二叉树平衡判定、顺序表奇偶位置重排、旋转数组最小值、二叉树镜像相似、奇数号元素稳定前移、顶点入度、孩子兄弟树的度和单链表中心对称判定；各卷第 41 题属于应用、手算或算法过程分析，不在目录中。

2026 年考试目前没有可用于逐题核验的公开原卷，公开回忆版之间仍有题面和分值冲突，因此没有以 `【真题（2026）】` 导入。这样可以避免把未经核实的回忆内容标成准确真题。

真题题库定义保存在 `backend/app/question_bank.py`，王道模拟题题库定义保存在 `backend/app/wangdao_mock_bank.py`。两者都包含固定随机种子和标准答案生成器，因此仓库本身即可在空数据库中重建全部 25 道代码题和 302 个测试点，不依赖本机数据库导出。Docker 卷中的个人提交记录、代码草稿和其他运行数据不会进入 Git 仓库。
...
GitHub Actions 会在推送到 `main`、创建 Pull Request 或手动触发时运行后端测试、前端生产构建和完整 Docker 冒烟测试。冒烟测试使用临时数据库核对 25 道受管题与 302 个测试点，并分别验证 C/C++ 提交能够通过沙箱判题；工作流结束后会删除 CI 容器和数据卷。
